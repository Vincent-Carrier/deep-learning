...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        output_error_term = error * self.activation_derivative(final_outputs)
        # TODO: Calculate the hidden layer's contribution to the error
        hidden_error = np.dot(output_error_term, self.weights_hidden_to_output.T)
        hidden_error_term = hidden_error * self.activation_derivative(hidden_outputs)
        # Weight step (input to hidden)
        delta_weights_i_h += hidden_error_term * hidden_outputs * 100
        # Weight step (hidden to output)
        delta_weights_h_o += output_error_term * final_outputs * 100
